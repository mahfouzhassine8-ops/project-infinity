#!/usr/bin/env python3
"""Run unchanged locked navigation gates and new health/preferences/recovery adapter tests."""
...
def main():
 ...
 shutil.copy2('view-delta/repairs/cobra-navigation-2103157/tests/CobraNavigationUiTest.java',test)
 shutil.copy2(Path(__file__).with_name('CobraHealthUiTest.java'),test)
 with (app/'build.gradle').open('a') as f:f.write('''
// Test dependencies only; never packaged in the separately verified release APK.
...
''')
 ...
 command=['./gradlew','--no-daemon','--console=plain',':xbmc:testReleaseUnitTest','--tests','com.projectinfinity.kodi.CobraNavigationUiTest','--tests','com.projectinfinity.kodi.CobraHealthUiTest','--stacktrace']
 ...
 print('PASS: production Android views/adapters tested; real provider/GPU/installation acceptance not claimed')
