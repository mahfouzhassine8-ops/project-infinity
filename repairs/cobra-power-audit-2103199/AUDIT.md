...
- Fit the existing guide-ruler labels to their actual half-hour slots. Preserve the original full label where it fits; use a bounded time label in narrow/large-text layouts, leaving ticks, guide geometry, dates above the ruler and transport unchanged. This baseline defect was found by inspecting final-candidate Android fixture renders after the earlier automated gates passed.
...
The inherited 265 Android cases remain mandatory, alongside 28 new cases (7 timeshift, 13 display, 8 UI). Behavioral suites and host evidence are separately enumerated in ACCEPTANCE.json and the evidence archive. Tests that only assert a selected aspect integer are not accepted as display coverage; every existing aspect mode is exercised through actual TextureView transform geometry. Physical pixel output remains a device gate. Some inherited drawer/sheet PNGs capture unfinished animations and direct preference writes; they are not settled whole-theme acceptance screenshots.
