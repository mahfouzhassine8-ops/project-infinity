...
OLD='1.0.9-Cobra-TS-Keyframe-Compatibility-RC1'
NEW='1.0.9-Cobra-TS-Timeline-Fingerprint-RC1'
VERSION=2103189
...
def upgrade():
    ...
    require(data.get('version_code')==2103188 and data.get('version_name')==OLD,'Expected exact passed 2103188 source receipt')
    for rel,row in data.get('files',{}).items():
        require(sha(Path('kodi')/rel)==row['after'],'2103188 source drift: '+rel)
    run('python3',ROOT/'apply.py','--source','kodi','--receipt',receipt,'--out','audit189/patch')
    patch=json.loads(Path('audit189/patch/patch.json').read_text()); data['files'][ACT]['after']=patch['files'][ACT]['after']

    gradle=Path('kodi')/GRADLE
    replace(gradle,'versionCode 2103188',f'versionCode {VERSION}')
    replace(gradle,'versionName "'+OLD+'"','versionName "'+NEW+'"')
    runtime=Path('scripts/infinity_background_resume.py')
    replace(runtime,'VERSION_CODE = 2103188',f'VERSION_CODE = {VERSION}')
    ...
    require(s.count(needle)>=2,'2103188 packager identity drift'); pack.write_text(s.replace(needle,'Infinity-'+NEW))

    data['files'][GRADLE]['after']=sha(gradle)
    data.update(version_code=VERSION,version_name=NEW,source_parent=2103188,source_parent_locked=True,locked_parent_commit=BASE_COMMIT,candidate_locked=False,
      ...
      stream_fingerprint_schema=4,cadence_diagnostics_preserved=True,frozen_live_snapshot=True,seamless_live_proxy=True,
      timeline_probe_only=True,pat_pmt_probe=True,video_pts_probe=True,video_dts_probe=True,audio_pts_probe=True,av_drift_probe=True,h264_nal_probe=True,intra_cadence_probe=True,rebuffer_timeline_correlation=True,
    ...
    for rel,row in data['files'].items():
        require(sha(Path('kodi')/rel)==row['after'],'Final 2103189 source drift: '+rel)
    run('python3',ROOT/'source_audit.py','--source','kodi','--patch','audit189/patch/patch.json','--out','audit189/source-audit')
    with Path('audit189/native-after.patch').open('wb') as out:
        subprocess.run(['git','-C','kodi','diff','--binary','--','xbmc'],stdout=out,check=True)
    require(Path('audit189/native-after.patch').read_bytes()==Path('engine/native-before.patch').read_bytes(),'Native source changed')
    print('PASS: exact 2103188 -> 2103189 TS timeline fingerprint; native tree preserved')

def verify():
    base=Path('baseline188/Infinity-'+OLD+'.apk'); final=Path('signed189/Infinity-'+NEW+'.apk')
    require(base.is_file() and final.is_file(),'APK missing')
    bv=json.loads(Path('baseline188/2103188-verification.json').read_text())
    require(bv['apk_sha256']==sha(base),'Wrong exact passed 2103188 APK')
    audit=json.loads(Path('signed189/background-resume-apk-audit.json').read_text())
    require(audit['version_code']==VERSION and audit['version_name']==NEW,'2103189 identity mismatch')
    ...
    with zipfile.ZipFile(base) as old,zipfile.ZipFile(final) as new:
        ...
        for token in (
          ...
          b'cobra_unified_live_timeline',b'CobraCallAudioPolicy'):
            require(token in dex,'2103189 contract missing: '+repr(token))
        require(b'Cobra2103187TsAccessUnitCompatibilityTest' not in dex and b'Cobra2103188TsKeyframeCompatibilityTest' not in dex and b'Cobra2103189TsTimelineFingerprintTest' not in dex,'Test code packaged')

    badging=Path('signed189/badging.txt').read_text()
    require("package: name='com.projectinfinity.kodi'" in badging and "versionCode='2103189'" in badging,'Not forward-installable')
    result={'build':VERSION,'base_build':2103188,'base_commit':BASE_COMMIT,'base_apk_sha256':sha(base),'apk_sha256':sha(final),'signer':CERT,
      'ts_extractor_profile':'DETECT_ACCESS_UNITS+ALLOW_NON_IDR_KEYFRAMES','ts_detect_access_units':True,'ts_allow_non_idr_keyframes':True,
      'stream_fingerprint_schema':4,'cadence_diagnostics_preserved':True,'frozen_live_snapshot':True,'timeline_probe_only':True,
    ...
    Path('audit189/final-verification.json').write_text(json.dumps(result,indent=2)+'\n')
    print('PASS: signed 2103189 preserves exact 2103188 native/resources/signer')
...
def deliver():
    ...
    total+=suite('audit189/android/runtime/test-results/TEST-com.projectinfinity.kodi.CobraVisualRuntimeTest.xml',17)
    total+=suite('audit189/android/runtime/test-results/TEST-com.projectinfinity.kodi.CobraVisualLayoutTest.xml',10)
    for name,count in [
      ...
      ('Cobra2103187TsAccessUnitCompatibilityTest',5),('Cobra2103188TsKeyframeCompatibilityTest',5),('Cobra2103189TsTimelineFingerprintTest',5)]:
        total+=suite(Path('audit189/targeted/test-results')/('TEST-com.projectinfinity.kodi.'+name+'.xml'),count)
    require(total==217,f'Expected 217 Android tests, got {total}')
    source=json.loads(Path('audit189/source-audit/source-audit.json').read_text())
    require(source.get('passed') and not source.get('failed'),'2103189 source audit failed')
    final=json.loads(Path('audit189/final-verification.json').read_text())
    result={'build':VERSION,'locked_parent':2103188,'locked_parent_commit':BASE_COMMIT,'android_tests':total,'new_ts_timeline_tests':5,
      'ts_extractor_profile':'DETECT_ACCESS_UNITS+ALLOW_NON_IDR_KEYFRAMES','ts_detect_access_units':True,'ts_allow_non_idr_keyframes':True,
      'stream_fingerprint_schema':4,'cadence_diagnostics_preserved':True,'frozen_live_snapshot':True,'timeline_probe_only':True,
      'pat_pmt_probe':True,'video_pts_probe':True,'video_dts_probe':True,'audio_pts_probe':True,'av_drift_probe':True,'h264_nal_probe':True,'intra_cadence_probe':True,'rebuffer_timeline_correlation':True,
      'playback_behavior_changed':False,'network_selection_changed':False,'timeshift_ownership_changed':False,'buffer_policy_changed':False,'parser_flags_changed':False,
      ...
      'status':'TEST CANDIDATE - observation-only TS/PES/H.264 timeline fingerprint for Problem Nicktoons before any timestamp rewrite'}
    Path('signed189/ACCEPTANCE.json').write_text(json.dumps(result,indent=2)+'\n')
    shutil.copy2('audit189/final-verification.json','signed189/2103189-verification.json')
    shutil.copy2('audit189/source-audit/source-audit.json','signed189/2103189-source-audit.json')
    print('PASS:',total,'Android tests + 2103189 TS-timeline gates')
