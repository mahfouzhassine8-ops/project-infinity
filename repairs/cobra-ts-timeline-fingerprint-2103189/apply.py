...
HELPERS=r'''    private void observeProgramMap(byte[] data,int packetOffset,int pid,int payloadOffset,int packetEnd,boolean pusi){
    ...
    private static int readUe(byte[] b,int[] bit,int bits){int zeros=0;while(bit[0]<bits){int v=(b[bit[0]>>3]>>(7-(bit[0]&7)))&1;bit[0]++;if(v!=0)break;if(++zeros>30)return -1;}int value=(1<<zeros)-1;for(int i=0;i<zeros;i++){if(bit[0]>=bits)return -1;value=(value<<1)|((b[bit[0]>>3]>>(7-(bit[0]&7)))&1);bit[0]++;}return value;}
