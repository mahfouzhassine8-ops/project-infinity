#!/usr/bin/env python3
"""Fast-pack the audited 2103235 RC15 final-polish layer over exact signed locked 2103234 RC14 ARMv7 APK.

No Kodi native rebuild, no apktool and no smali. Java/manifest/resources are
compiled from source, resource IDs are pinned to the protected 2103234 RC14 APK, and
only compiled DEX + manifest replace their counterparts. All native libraries,
assets and compiled Android resources remain byte-identical to 2103234.
"""
...
VERSION=2103235
RELEASE='1.0.9-Cobra-Onn4KPro-TV-Final-Polish-RC15'
BASE_APK_SHA256='5678310d74ffd17e72badaf7cc3f7bbfaeb3371b151bb524bdc39c31e052018d'
...
def source_preservation(source:Path):
    ...
    require(receipt.get('version_code')==VERSION and receipt.get('version_name')==RELEASE,'Wrong audited 2103235 source receipt')
...
def prepare(source:Path,base:Path,build:Path,out:Path):
    require(sha(base.read_bytes())==BASE_APK_SHA256,'Not exact locked 2103234 RC14 signed TV APK')
...
def main():
    ...
    unsigned=out/'Infinity-1.0.9-Cobra-Onn4KPro-TV-Final-Polish-RC15-unsigned.apk'
    ...
    final=out/'Infinity-1.0.9-Cobra-Onn4KPro-TV-Final-Polish-RC15.apk';run('bash',ROOT/'scripts/sign-infinity71.sh',unsigned,final)
    ...
    (out/'ONN-TV-2103235-VERIFICATION.json').write_text(json.dumps(report,indent=2,sort_keys=True)+'\n')
    shutil.copy2(ROOT/'engine/background-resume-source.json',out/'background-resume-source.json')
    shutil.copy2(ROOT/'audit235/tv-final-polish-source.json',out/'tv-final-polish-source.json')
    unsigned.unlink()
    print('PASS: 2103235 RC15 final TV polish layer + exact locked 2103234 RC14 APK payload/native/resources; permanent signer verified')
