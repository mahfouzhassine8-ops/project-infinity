...
V=2103236
NAME='1.0.9-Cobra-All-Views-Playing-Indicator-RC1'
PARENT_SHA='6a82289548559d0d9a4c9ffca995570e7c93c55adba8f2a31d0ae8dbc7f7df98'
    ...
    for f in directory.glob('TEST-*.xml'):
      ...
      for case in doc.findall('.//testcase'):
        ...
        req(case.find('failure') is None and case.find('error') is None and case.find('skipped') is None,
            'focused indicator test failed/skipped: '+cls+'.'+case.attrib.get('name',''))
...
def main():
    ...
    p.add_argument('--apk',type=Path,required=True);p.add_argument('--parent235',type=Path,required=True)
    ...
    source=json.loads(a.source_audit.read_text());inherited=json.loads(a.inherited_result.read_text())
    ...
    req(h(a.parent235.read_bytes())==PARENT_SHA,'not exact passed 2103235 parent APK')
    req(source.get('passed') is True and source.get('views')==['grid','mobile','compact','cards','focus'],'all-view source audit failed')
    ...
    with zipfile.ZipFile(a.parent235) as pz,zipfile.ZipFile(a.apk) as z:
      ...
      for token in (
        b'CobraPlayingIndicatorPolicy',b'CobraMobileChannelRow',b'CobraCompactChannelRow',
    ...
    out={
      'build':V,'parent':2103235,'apk_sha256':h(a.apk.read_bytes()),'parent_apk_sha256':PARENT_SHA,
      ...
      'tv_grid_renderer_preserved':True,'night_cinema_color':'warm amber/yellow #FFC247',
      'ambient_adaptive':True,'physical_device_verified':False,
      'status':'TEST CANDIDATE — automated gates passed; device visual/session verification required'
    }
    Path('signed236/ACCEPTANCE.json').write_text(json.dumps(out,indent=2)+'\n')
