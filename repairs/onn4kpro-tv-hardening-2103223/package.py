#!/usr/bin/env python3
"""Fast-pack the audited 2103223 hardened Android TV layer over exact signed 2103221 ARMv7 APK.
...
VERSION=2103223
RELEASE='1.0.9-Cobra-Onn4KPro-TV-Hardening-RC6'
...
def source_preservation(source:Path):
    ...
    require(receipt.get('tv_hardening') is True,'2103223 hardening receipt missing')
    require(receipt.get('tv_guide_layout_rebuild_loop_fixed') is True and receipt.get('tv_cache_restore_off_main') is True,'2103223 performance gates missing')
    require(receipt.get('tv_transient_focus_owned') is True and receipt.get('tv_grid_focus_graph') is True,'2103223 focus gates missing')
...
def verify_bytes(base:Path,final:Path):
    with zipfile.ZipFile(base) as a,zipfile.ZipFile(final) as b:
        ...
        for token in (b'cobra_tv_grid_only_2103220',b'cobra_tv_remote_ui_2103221',b'cobra_tv_player_multiview_2103222',b'cobra_tv_hardening_2103223',
                      b'Return to Multi-View',b'Search and add',b'Enlarge screen',b'cobra_tv_player_tool_channels',b'Opening saved TV library',b'TV SYSTEM'):
...
def main():
    ...
    unsigned=out/'Infinity-1.0.9-Cobra-Onn4KPro-TV-Hardening-RC6-unsigned.apk'
    ...
    final=out/'Infinity-1.0.9-Cobra-Onn4KPro-TV-Hardening-RC6.apk';run('bash',ROOT/'scripts/sign-infinity71.sh',unsigned,final)
    ...
    report={'schema':1,'build':VERSION,'version_name':RELEASE,'package':PACKAGE,'target_abi':'armeabi-v7a','hardening':True,
    ...
    (out/'ONN-TV-2103223-VERIFICATION.json').write_text(json.dumps(report,indent=2,sort_keys=True)+'\n')
    shutil.copy2(ROOT/'engine/background-resume-source.json',out/'background-resume-source.json')
    shutil.copy2(ROOT/'audit223/tv-hardening-source.json',out/'tv-hardening-source.json')
    unsigned.unlink()
    print('PASS: 2103223 hardened TV Android layer + exact locked 2103221 ARMv7 native/resources; permanent signer verified')
