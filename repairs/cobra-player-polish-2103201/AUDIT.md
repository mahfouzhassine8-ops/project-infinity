...
The `421dc5b` builds passed all 357 automated cases but were withheld after visual review showed a newly discovered English row clipped by the anchored panel's cached initial height. Track refresh now releases only that current panel back to content measurement before the existing anchor/viewport owner reapplies its bounds. The same panel, scroll container and focus remain; no reopening or entry-animation restart is added. An extracted Android owner reproduction changed the visible row from 30/58 pixels to 58/58 and verifies reverse shrink and bounded long-list scrolling. The strengthened complete-Activity tests check actual visible bounds, growth/shrink, preserved focus/scroll and many tracks. There are now 358 cases expected in final CI, including 15 subtitle cases. Existing focus enlargement can clip its horizontal outline at item bounds; that inherited styling limitation is documented rather than globally disabling scroll clipping.
