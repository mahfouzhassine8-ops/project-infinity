...
def main():
    ...
    chrome=block(text,'cobraBuildPlayerChrome');player_settings=block(text,'showPlayerSettingsDrawer')
    ...
    pref=block(text,'CobraPreferencePolicy','class');channel_mode=block(text,'cobraChannelAspect');defaults=block(text,'cobraShowPlaybackDefaults')
    checks={
      'exact_2103194_parent':patch.get('base_build')==2103194 and patch.get('base_commit')=='2c2a759bd7f1ef380f21609105675ff2012d4d56',
      'original_player_menu':all(x in player_settings for x in ['Player settings','Viewing preferences and recording','Channel playback','Health Center','Audio & subtitles','Aspect / Display','Cast / Route','Manage sources','Close player']),
      'original_menu_not_rewritten':patch.get('player_settings_body_preserved') is True,
      'no_player_options_hub':'showCobraPlayerOptionsHub' not in text and 'cobraAddRecentPlayerStrip' not in text and 'RECENT CHANNELS' not in text and 'cobra-player-hub-' not in text,
      'no_video_display_submenu':'showCobraVideoOptions' not in text and 'Video & display' not in player_settings,
      'original_four_button_toolbar':'String[] glyphs={"guide","aspect","multi","more"},labels={"Channels","Display","Multi-View","More"};' in chrome and 'expandedTools' not in chrome and '"Options"' not in chrome,
      'player_chrome_motion':'chrome.setTranslationY(dp(8))' in chrome and 'DecelerateInterpolator' in chrome,
      'sheet_motion':'setScaleX(.985f)' in sheet and 'setScaleY(.985f)' in sheet and 'DecelerateInterpolator' in sheet,
      'focus_motion':all(x in block(text,'cobraPolishFocusable') for x in ['setOnFocusChangeListener','focused?1.018f:1f','CobraMotionSpec.MICRO','DecelerateInterpolator']),
    ...
    result={'passed':not failed,'checks':checks,'failed':failed,'build':2103197,'surgical_player_menu_revert':True,'non_menu_polish_preserved':True,'physical_device_verified':False}
    ...
    print('PASS:',len(checks),'2103197 surgical player-menu source checks')
