#!/usr/bin/env python3
"""Package/gate 2103184 stream fingerprint + seamless rewind + smart dual-stack over exact passed 2103183."""
...
OLD='1.0.9-Cobra-Network-Family-Control-RC1'
NEW='1.0.9-Cobra-Stream-Network-Optimization-RC1'
VERSION=2103184
...
def replace(path,old,new,count=1):
 p=Path(path);s=p.read_text();actual=s.count(old);require(actual==count,f'identity anchor drift {path}: {old} count={actual}');p.write_text(s.replace(old,new,count) if count==1 else s.replace(old,new))
def run(*args):subprocess.run(list(map(str,args)),check=True)
def upgrade():
 receipt=Path('engine/background-resume-source.json');data=json.loads(receipt.read_text())
 require(data.get('version_code')==2103183 and data.get('version_name')==OLD,'Expected exact passed 2103183 source receipt')
 for rel,row in data.get('files',{}).items():require(sha(Path('kodi')/rel)==row['after'],'2103183 source receipt drift: '+rel)
 run('python3',ROOT/'apply.py','--source','kodi','--receipt',receipt,'--out','audit184/patch')
 patch=json.loads(Path('audit184/patch/patch.json').read_text());data['files'][ACT]['after']=patch['files'][ACT]['after'];data['files'][GRADLE]['after']=patch['files'][GRADLE]['after']
 gradle=Path('kodi')/GRADLE;replace(gradle,'versionCode 2103183',f'versionCode {VERSION}');replace(gradle,'versionName "'+OLD+'"','versionName "'+NEW+'"')
 runtime=Path('scripts/infinity_background_resume.py');replace(runtime,'VERSION_CODE = 2103183',f'VERSION_CODE = {VERSION}');replace(runtime,"RELEASE = '"+OLD+"'","RELEASE = '"+NEW+"'")
 ...
 for rel,row in data['files'].items():require(sha(Path('kodi')/rel)==row['after'],'Final 2103184 source receipt drift: '+rel)
 run('python3',ROOT/'source_audit.py','--source','kodi','--patch','audit184/patch/patch.json','--out','audit184/source-audit')
 with Path('audit184/native-after.patch').open('wb') as out:subprocess.run(['git','-C','kodi','diff','--binary','--','xbmc'],stdout=out,check=True)
 require(Path('audit184/native-after.patch').read_bytes()==Path('engine/native-before.patch').read_bytes(),'Native source changed')
 print('PASS: exact 2103183 -> 2103184 stream/network optimization; native tree preserved')
def verify():
 base=Path('baseline183/Infinity-'+OLD+'.apk');final=Path('signed184/Infinity-'+NEW+'.apk');require(base.is_file() and final.is_file(),'Baseline/final APK missing')
 bv=json.loads(Path('baseline183/2103183-verification.json').read_text());require(bv['apk_sha256']==sha(base),'Wrong exact passed 2103183 APK')
 audit=json.loads(Path('signed184/background-resume-apk-audit.json').read_text());require(audit['version_code']==VERSION and audit['version_name']==NEW,'Final 2103184 identity mismatch');require(audit['signer_certificate_sha256']==CERT and audit['apk_sha256']==sha(final),'Signer/hash mismatch');require(not audit['native_recompiled'],'Native engine was rebuilt')
 deps=Path('signed184/android-dependencies.txt').read_text();require('androidx.media3:media3-datasource-okhttp:1.7.1' in deps,'Media3 OkHttp transport not packaged');require('com.squareup.okhttp3:okhttp:' in deps,'OkHttp runtime dependency missing')
...
def suite(path,count):
 p=Path(path);require(p.is_file(),'Missing test evidence '+str(p));root=ET.parse(p).getroot();cases=root.findall('testcase');require(len(cases)==count,f'Unexpected test count {p}: {len(cases)} != {count}');require(all(int(root.get(k,'0'))==0 for k in ('failures','errors','skipped')),'Failed/incomplete suite '+str(p));require(all(c.find(k) is None for c in cases for k in ('failure','error','skipped')),'Failed/skipped testcase '+str(p));return count
def deliver():
 inherited=0
