...
def patch_live(s):
    ...
    def append_before_close(block,line):
        ...
        # Keep the original closing-brace line byte-identical for lifecycle audits.
        closing_line=block.rfind("\n",0,i)+1
        if block[closing_line:i].strip():
            raise RuntimeError("method closing brace must be on its own line")
        return block[:closing_line]+line+"\n"+block[closing_line:]
