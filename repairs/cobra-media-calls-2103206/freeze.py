...
def collect_tests():
    tests, hashes = {}, {}
    paths = sorted((ROOT / 'tests').glob('Cobra2103206*.java'))
...
def freeze(parent, candidate, rollback_receipt):
    ...
    require(len(complete) == 226 and len(expected) == 27, 'Parent inventory drift')
    ...
    for name in sorted(new_inventory):
        ...
        if before == after:
            continue
        if name == GRADLE:
            wanted = (parent / name).read_text().replace('versionCode 2103205', 'versionCode 2103206')
            wanted = wanted.replace('1.0.9-Cobra-Final-Features-RC1', '1.0.9-Cobra-Media-Calls-RC1')
    ...
    previous = ROOT.parent / 'cobra-final-features-2103205'
    inherited = (json.loads((previous / 'inherited-android-cases.json').read_text()) |
                 json.loads((previous / 'new-android-cases.json').read_text()))
    require(len(inherited) == 64 and sum(map(len, inherited.values())) == 637, 'Parent Android inventory drift')
    ...
    reviewed = {
        'status': 'frozen', 'parent_build': 2103205, 'parent_commit': PARENT_COMMIT,
        ...
        'parent_source_files': 226, 'new_source_files': len(additions),
        'protected_runtime_members': protected, 'inherited_android_cases': 637,
        ...
        'scope': 'Android audio policy, existing transport state, one requested setting; no native/transport rewrite',
    }
    ...
    print(json.dumps({'status': 'frozen', 'source_files': sorted(files), 'protected_members': len(protected),
                      'inherited_cases': 637, 'new_cases': reviewed['new_android_cases'],
