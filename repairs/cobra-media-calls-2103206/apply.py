"""Apply the frozen, user-requested Java extension over exact locked 2103205."""
...
PARENT_COMMIT = 'd4abe960873929ebee3f13af6a976c19011d0cdc'
RUNTIME_ALLOWLIST = frozenset([ACTIVITY])
PROTECTED_CLASSES = (
...
PROTECTED_METHODS = (
    ...
    'cobraDispatchPipCommand', 'cobraUpdatePipMediaSession',
...
def checked_review():
    ...
    require(reviewed.get('parent_build') == 2103205 and reviewed.get('parent_commit') == PARENT_COMMIT,
    ...
    for name, digest in reviewed['test_source_hashes'].items():
        require(re.fullmatch(r'tests/Cobra2103206[A-Za-z0-9]+\.java', name) is not None,
                'Invalid frozen test path')
        require(sha(ROOT / name) == digest, 'Frozen test drift: ' + name)
    require(reviewed['inherited_android_cases'] == 637, 'Inherited identity count drift')
    require(reviewed['parent_source_files'] == 226, 'Parent source inventory drift')
...
def apply(source, receipt, out):
    ...
    require(data['version_code'] == 2103205, 'Wrong parent version')
    require(len(expected) == 27 and len(complete) == 226, 'Parent inventories changed')
    ...
    with zipfile.ZipFile(out / 'pre-change-android-source.zip', 'x', zipfile.ZIP_DEFLATED) as archive:
        ...
        require(len(archive.namelist()) == len(set(archive.namelist())) == 226,
    ...
    result = {
        'parent_build': 2103205, 'parent_commit': PARENT_COMMIT,
        'files': reviewed['files'], 'protected_runtime_members': preserved,
        'parent_source_files': 226, 'final_source_files': len(wanted),
