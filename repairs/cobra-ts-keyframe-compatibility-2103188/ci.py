...
OLD='1.0.9-Cobra-TS-Access-Unit-Compatibility-RC1'
NEW='1.0.9-Cobra-TS-Keyframe-Compatibility-RC1'
VERSION=2103188
...
def upgrade():
    ...
    replace(gradle,'versionCode 2103187',f'versionCode {VERSION}')
    replace(gradle,'versionName "'+OLD+'"','versionName "'+NEW+'"')
    runtime=Path('scripts/infinity_background_resume.py')
    replace(runtime,'VERSION_CODE = 2103187',f'VERSION_CODE = {VERSION}')
    ...
    require(s.count(needle)>=2,'2103187 packager identity drift'); pack.write_text(s.replace(needle,'Infinity-'+NEW))

    data['files'][GRADLE]['after']=sha(gradle)
    data.update(version_code=VERSION,version_name=NEW,source_parent=2103187,source_parent_locked=True,locked_parent_commit=BASE_COMMIT,candidate_locked=False,
    ...
    for rel,row in data['files'].items():
        require(sha(Path('kodi')/rel)==row['after'],'Final 2103188 source drift: '+rel)
    run('python3',ROOT/'source_audit.py','--source','kodi','--patch','audit188/patch/patch.json','--out','audit188/source-audit')
    with Path('audit188/native-after.patch').open('wb') as out:
        subprocess.run(['git','-C','kodi','diff','--binary','--','xbmc'],stdout=out,check=True)
    require(Path('audit188/native-after.patch').read_bytes()==Path('engine/native-before.patch').read_bytes(),'Native source changed')
    print('PASS: exact 2103187 -> 2103188 TS keyframe compatibility; native tree preserved')
...
def verify():
    ...
    require("package: name='com.projectinfinity.kodi'" in badging and "versionCode='2103188'" in badging,'Not forward-installable')
    result={'build':VERSION,'base_build':2103187,'base_commit':BASE_COMMIT,'base_apk_sha256':sha(base),'apk_sha256':sha(final),'signer':CERT,
      'ts_extractor_profile':'DETECT_ACCESS_UNITS+ALLOW_NON_IDR_KEYFRAMES','ts_detect_access_units':True,'ts_allow_non_idr_keyframes':True,
    ...
    Path('audit188/final-verification.json').write_text(json.dumps(result,indent=2)+'\n')
    print('PASS: signed 2103188 preserves exact 2103187 native/resources/signer')
...
def deliver():
    ...
    total+=suite('audit188/android/runtime/test-results/TEST-com.projectinfinity.kodi.CobraVisualRuntimeTest.xml',17)
    total+=suite('audit188/android/runtime/test-results/TEST-com.projectinfinity.kodi.CobraVisualLayoutTest.xml',10)
    for name,count in [
      ...
      ('Cobra2103187TsAccessUnitCompatibilityTest',5),('Cobra2103188TsKeyframeCompatibilityTest',5)]:
        total+=suite(Path('audit188/targeted/test-results')/('TEST-com.projectinfinity.kodi.'+name+'.xml'),count)
    require(total==212,f'Expected 212 Android tests, got {total}')
    source=json.loads(Path('audit188/source-audit/source-audit.json').read_text())
    require(source.get('passed') and not source.get('failed'),'2103188 source audit failed')
    final=json.loads(Path('audit188/final-verification.json').read_text())
    result={'build':VERSION,'locked_parent':2103187,'locked_parent_commit':BASE_COMMIT,'android_tests':total,'new_ts_keyframe_tests':5,
      'ts_extractor_profile':'DETECT_ACCESS_UNITS+ALLOW_NON_IDR_KEYFRAMES','ts_detect_access_units':True,'ts_allow_non_idr_keyframes':True,
      'stream_fingerprint_schema':3,'cadence_diagnostics_preserved':True,'frozen_live_snapshot':True,
      ...
      'status':'TEST CANDIDATE - final Problem Nicktoons IPv4 Only check for the remaining documented Media3 H.264 TS keyframe compatibility path'}
    Path('signed188/ACCEPTANCE.json').write_text(json.dumps(result,indent=2)+'\n')
    shutil.copy2('audit188/final-verification.json','signed188/2103188-verification.json')
    shutil.copy2('audit188/source-audit/source-audit.json','signed188/2103188-source-audit.json')
    print('PASS:',total,'Android tests + 2103188 TS-keyframe gates')
