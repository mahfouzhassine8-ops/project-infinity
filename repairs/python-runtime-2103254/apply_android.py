#!/usr/bin/env python3
"""2103254: Python runtime repair Android/diagnostic delta over exact locked 2103229 source."""
...
NEW_NAME = "1.0.9-Python-Runtime-Native-Repair-RC1"
...
''',
    ...
    p = p.replace(
        ...
        "Infinity-1.0.9-Python-Runtime-Native-Repair-RC1-unsigned.apk",
    )
    p = p.replace(
        "Infinity-1.0.9-Cobra-MultiView-Stability-Fill-RC1.apk",
        "Infinity-1.0.9-Python-Runtime-Native-Repair-RC1.apk",
    )
    req(
...
def main():
    ...
    receipt.update(
        ...
        native_engine_reused_from_2103209=False,
        native_engine_parent_sha256=NATIVE_ENGINE_SHA,
    ...
    scope = {
        ...
        "native_engine_rebuilt": True,
        "authorized_delta": [
            "rebuilt libkodi.so with PythonInvoker failure/teardown repair",
        ...
        "status": "TEST CANDIDATE — native Python runtime repair",
    }
    (audit / "scope.json").write_text(json.dumps(scope, indent=2, sort_keys=True) + "\n")
    print("PASS: 2103254 Android/diagnostic delta applied to exact locked 2103229 source")
