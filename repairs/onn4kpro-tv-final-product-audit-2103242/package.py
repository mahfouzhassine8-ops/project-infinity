#!/usr/bin/env python3
"""Fast-pack the audited 2103242 RC22 final-product-audit layer over exact signed locked 2103241 RC21 ARMv7 APK.
...
VERSION=2103242
RELEASE='1.0.9-Cobra-Onn4KPro-TV-Final-Product-Audit-RC22'
BASE_APK_SHA256='e1ebde8a5f6477c77431139b351140b54417f662735caeec77d78d14313b6261'
...
def source_preservation(source:Path):
    ...
    require(receipt.get('version_code')==VERSION and receipt.get('version_name')==RELEASE,'Wrong audited 2103242 source receipt')
    ...
    require(receipt.get('tv_player_chrome_auto_hide_restored') is True and receipt.get('tv_player_chrome_uses_existing_delay_policy') is True,'2103241 chrome timeout contract missing')
    require(receipt.get('tv_player_back_first_hides_chrome') is True and receipt.get('tv_player_back_second_exits') is True,'2103241 Back hierarchy missing')
    require(receipt.get('tv_channel_playback_linear_focus') is True and receipt.get('tv_channel_playback_single_initial_focus_post') is True,'2103241 Channel playback focus contract missing')
    require(receipt.get('tv_transport_toggle_guard_ms')==220 and receipt.get('tv_transport_duplicate_toggle_suppressed') is True,'2103241 transport debounce missing')
    require(receipt.get('tv_unexpected_non_user_pause_recovery') is True and receipt.get('tv_unexpected_pause_no_restart') is True,'2103241 unexpected pause safety missing')
    require(receipt.get('tv_buffering_policy_observation_only') is True and receipt.get('tv_buffering_media_restart_forbidden') is True and receipt.get('tv_timeshift_engine_unchanged') is True,'2103241 buffer/timeshift preservation missing')
    require(receipt.get('tv_final_product_audit') is True and receipt.get('tv_shipping_preservation_audit') is True,'2103242 final product audit receipt missing')
    require(receipt.get('tv_power_menu_remote_focus') is True and receipt.get('tv_power_menu_initial_focus') is True and receipt.get('tv_power_menu_linear_focus') is True,'2103242 Power focus contract missing')
    require(receipt.get('tv_power_menu_transient_owner') is True and receipt.get('tv_power_menu_back_cancel') is True and receipt.get('tv_power_menu_safe_focus_restore') is True,'2103242 Power transient contract missing')
    require(receipt.get('tv_grid_legacy_playing_text_removed') is True and receipt.get('tv_playing_dot_authoritative') is True,'2103242 playing indicator cleanup missing')
...
def prepare(source:Path,base:Path,build:Path,out:Path):
    require(sha(base.read_bytes())==BASE_APK_SHA256,'Not exact locked 2103241 RC21 signed TV APK')
...
def main():
    ...
    unsigned=out/'Infinity-1.0.9-Cobra-Onn4KPro-TV-Final-Product-Audit-RC22-unsigned.apk'
    ...
    final=out/'Infinity-1.0.9-Cobra-Onn4KPro-TV-Final-Product-Audit-RC22.apk';run('bash',ROOT/'scripts/sign-infinity71.sh',unsigned,final)
    ...
    report={'schema':1,'build':VERSION,'version_name':RELEASE,'package':PACKAGE,'target_abi':'armeabi-v7a','hardening':True,'native_remote_architecture':True,'handoff_first_render':True,'multiview_focus_visible':True,'player_footer_unclipped':True,'multiview_two_to_one_session_preserved':True,'navigation_hierarchy':'drawer-groups-grid-player','back_reverses_hierarchy':True,'mini_player_preserved':True,'preview_engine_preserved':True,'player_channels_control_preserved':True,'group_counts_cached':True,'hamburger_removed':True,'grid_group_dropdown_removed':True,'settings_next_to_search':True,'mini_player_adaptive_large':True,'drawer_compact_integrated':True,'drawer_stage_shift':False,'drawer_destination_auto_close':True,'live_flow':'drawer-live-tv-groups-grid','glass_system':True,'glass_static_gradient':True,'real_time_blur':False,'ambient_mode_retired':True,'ambient_setting_exposed':False,'glass_drawer':True,'glass_playlist':True,'glass_guide':True,'glass_preview':True,'glass_sheets':True,'glass_player_surfaces':True,'glass_multiview':True,'drawer_inline_column':True,'drawer_guide_overlay':False,'drawer_back_restores_underlay':True,'drawer_groups_visible_while_open':True,'playing_channel_indicator':True,'playing_indicator_actual_session':True,'preview_instruction_removed':True,'border_density_reduced':True,'drawer_brand_compact':True,'drawer_brand_single_line':True,'drawer_power_unclipped':True,'live_right_select_drawer':True,'live_right_select_directory':True,'right_select_scope':'live-tv-only','vod_right_navigation_untouched':True,'movies_dedicated_search':True,'shows_dedicated_search':True,'mini_player_playing_badge_removed':True,'vod_session_cache_ms':300000,'performance_polish':True,'stability_polish':True,'ui_polish':True,'grid_pulsing_playing_dot':True,'playing_dot_actual_session':True,'playing_dot_focus_independent':True,'playing_dot_normal_pulse_ms':1320,'playing_dot_night_cinema_pulse_ms':1900,'playing_dot_handoff_ms':180,'playing_dot_color_morph_ms':240,'playing_dot_oled_core_floor_alpha':218,'playing_dot_oled_glow_floor':.58,'playing_dot_night_cinema_amber':'0xffffc247','playing_dot_ambient_independent':True,'playing_dot_frame_ms':33,'playing_dot_no_blur':True,'chooser_card_settings_removed':True,'chooser_infinity_settings_removed':True,'chooser_cobra_settings_removed':True,'cobra_explicit_start_destination':'live_tv','cobra_startup_integrated_shell':True,'cobra_startup_drawer_open':True,'cobra_startup_groups_visible':True,'drawer_right_select':True,'drawer_left_back':True,'directory_right_select':True,'directory_left_back':True,'left_back_scope':'live-tv-drawer-directory-only','section_owner_persistent':True,'section_owner_values':['LIVE_TV','MOVIES','SHOWS'],'movies_persistent_until_drawer_change':True,'shows_persistent_until_drawer_change':True,'live_persistent_until_drawer_change':True,'smart_return_cannot_override_vod_owner':True,'vod_player_origin_explicit':True,'vod_channels_control_removed':True,'vod_multiview_control_removed':True,'vod_channel_stepping_removed':True,'vod_last_channel_removed':True,'vod_channel_favorite_removed':True,'vod_live_rewind_removed':True,'vod_go_live_removed':True,'vod_live_timeline_removed':True,'vod_live_record_removed':True,'vod_epg_labels_removed':True,'vod_display_preserved':True,'vod_more_preserved':True,'vod_audio_subtitles_preserved':True,'vod_remote_seek_ms':30000,
            ...
            'player_chrome_auto_hide_restored':True,'player_chrome_existing_delay_policy':True,'player_menu_holds_chrome':True,
            'player_back_first_hides_chrome':True,'player_back_second_exits':True,
            'channel_playback_linear_focus':True,'channel_playback_single_initial_focus_post':True,'sheet_stale_focus_cancelled':True,
            'transport_toggle_guard_ms':220,'transport_duplicate_toggle_suppressed':True,
            'unexpected_non_user_pause_recovery':True,'unexpected_pause_recovery_delay_ms':280,'unexpected_pause_no_restart':True,
            'buffering_policy_observation_only':True,'buffering_media_restart_forbidden':True,'timeshift_engine_unchanged':True,
            'final_product_audit':True,'shipping_preservation_audit':True,
            'power_menu_remote_focus':True,'power_menu_initial_focus':True,'power_menu_linear_focus':True,
            'power_menu_transient_owner':True,'power_menu_back_cancel':True,'power_menu_safe_focus_restore':True,
            'grid_legacy_playing_text_removed':True,'grid_legacy_paused_text_removed':True,'playing_dot_authoritative':True,
            'ui_polish_preservation_first':True,
            'live_end_to_end_polish':True,'live_structure_unchanged':True,'motion_lightweight':True,'real_time_blur':False,
            'base_build':2103241,'base_apk_sha256':BASE_APK_SHA256,'base_native_engine_sha256':BASE_ENGINE_SHA256,
    ...
    (out/'ONN-TV-2103242-VERIFICATION.json').write_text(json.dumps(report,indent=2,sort_keys=True)+'\n')
    shutil.copy2(ROOT/'engine/background-resume-source.json',out/'background-resume-source.json')
    shutil.copy2(ROOT/'audit242/final-product-audit-source.json',out/'final-product-audit-source.json')
    unsigned.unlink()
    print('PASS: 2103242 RC22 final preservation-first TV product audit over exact locked 2103241 RC21 APK payload/native/resources; permanent signer verified')
