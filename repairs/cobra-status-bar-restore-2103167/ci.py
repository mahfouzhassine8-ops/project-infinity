#!/usr/bin/env python3
"""Locked 2103166 -> isolated 2103167 Android status-bar restore gates."""
from pathlib import Path
import argparse,hashlib,json,shutil,subprocess,zipfile,xml.etree.ElementTree as ET

ROOT=Path(__file__).resolve().parent
LOCKED_BASE='644160d0ca3baeebb6759638d3f40e2853eeed0b'
CANDIDATE14='1465eabb045badad56142642c48292df94caaa12'
OLD='1.0.9-Cobra-End-to-End-UI-Lifecycle-Audit-RC1'
NEW='1.0.9-Cobra-Status-Bar-Restore-RC1'
VERSION=2103167
...
def baseline_identity():
    apk=Path('baseline166/Infinity-'+OLD+'.apk')
    proof=Path('baseline166/2103166-verification.json')
    require(apk.is_file() and proof.is_file(),'Locked 2103166 artifact incomplete')
    data=json.loads(proof.read_text())
    require(data.get('version_code')==2103166,'Locked 2103166 version mismatch')
    require(data.get('apk_sha256')==sha(apk),'Locked 2103166 APK identity mismatch')
    return apk,data

def upgrade():
    base,baseproof=baseline_identity()
    out=Path('audit167');out.mkdir(exist_ok=True)
    ...
    replace('kodi/tools/android/packaging/xbmc/build.gradle.in','versionCode 2103166','versionCode 2103167')
    replace('kodi/tools/android/packaging/xbmc/build.gradle.in','versionName "'+OLD+'"','versionName "'+NEW+'"')
    replace('scripts/infinity_background_resume.py','VERSION_CODE = 2103166','VERSION_CODE = 2103167')
    ...
    require(data.get('version_code')==2103166,'Expected exact locked 2103166 source parent')
    ...
    data.update(
        ...
        source_parent=2103166,source_parent_commit=LOCKED_BASE,source_parent_locked=True,
    ...
    (out/'baseline166.json').write_text(json.dumps({'apk_sha256':sha(base),'verification':baseproof},indent=2)+'\n')
    print('PASS: exact locked 2103166 -> isolated 2103167 status-bar delta')

def verify():
    baseline_identity()
    apk=Path('signed167/Infinity-'+NEW+'.apk')
    audit=json.loads(Path('signed167/background-resume-apk-audit.json').read_text())
    ...
    manifest=Path('signed167/manifest.txt').read_text()
    ...
    Path('audit167/apk-verification.json').write_text(json.dumps(proof,indent=2)+'\n')
    print('PASS: signer/native/resources preserved against locked 2103166 and Candidate 14')
...
def deliver():
    ...
    for name,count in [('CobraVisualRuntimeTest',17),('CobraVisualLayoutTest',10)]:
        inherited+=suite(Path('audit167/android/runtime/test-results')/('TEST-com.projectinfinity.kodi.'+name+'.xml'),count)

    candidate14=suite('audit167/targeted/test-results/TEST-com.projectinfinity.kodi.Cobra2103162ThemeRotationTest.xml',16)
    locked164=suite('audit167/targeted/test-results/TEST-com.projectinfinity.kodi.Cobra2103164PlaybackStabilityTest.xml',26)
    locked165=suite('audit167/targeted/test-results/TEST-com.projectinfinity.kodi.Cobra2103165InsetsPipPlayerTest.xml',6)
    locked166=suite('audit167/targeted/test-results/TEST-com.projectinfinity.kodi.Cobra2103166EndToEndUiAuditTest.xml',8)
    new=suite('audit167/targeted/test-results/TEST-com.projectinfinity.kodi.Cobra2103167StatusBarRestoreTest.xml',5)
    ...
    apkproof=json.loads(Path('audit167/apk-verification.json').read_text())
    require(apkproof['apk_sha256']==sha('signed167/Infinity-'+NEW+'.apk'),'APK changed after verification')

    total=inherited+candidate14+locked164+locked165+locked166+new
    require(total==107,'Expected 107 Android tests, got '+str(total))
    report={
        ...
        'locked_2103166_tests':locked166,'new_2103167_tests':new,'total_android_tests':total,
        ...
        'status':'TEST CANDIDATE - physical Fold status-bar visibility confirmation required'
    }
    Path('signed167/ACCEPTANCE.json').write_text(json.dumps(report,indent=2)+'\n')
    shutil.copy2('audit167/apk-verification.json','signed167/2103167-verification.json')
    shutil.copy2('audit167/source-audit/source-audit.json','signed167/2103167-source-audit.json')
    shutil.copy2('audit167/patch/patch.json','signed167/2103167-source-proof.json')
    shutil.copy2(ROOT/'AUDIT.md','signed167/AUDIT-2103167.md')
    print('PASS:',total,'Android tests +',len(source['checks']),'source checks; locked 2103166 preserved')
