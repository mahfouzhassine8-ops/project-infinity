...
status bar cannot reveal Cobra when Cobra is not laid out behind that region.

This repair keeps the real status bar visible while making the window explicitly edge-to-edge.
The locked 2103166 mRoot inset listener remains the sole content-safe-area owner, so content stays
below status/cutout insets while the root background can actually paint behind the system bar.
"""
...
def apply(source,receipt_path,out):
    ...
    bars=old_bars.replace(
        ...
        'decor.setSystemUiVisibility(flags);cobraConfigureEdgeToEdgeSystemBars(window,decor);'
    ).replace(
        'window.setStatusBarColor(barColor);window.setNavigationBarColor(barColor);',
...
'''
    if method_matches(text,"cobraBrowseSystemBarSurfaceColor") or method_matches(text,"cobraConfigureEdgeToEdgeSystemBars"):
        raise RuntimeError("2103169 helper collision")
    ...
    edge=method_text(text,"cobraConfigureEdgeToEdgeSystemBars")
    for token in (
        ...
        if token not in after_bars:raise RuntimeError("2103169 bar contract missing "+token)
    for token in (
    ...
    for token in (
        ...
        if token not in edge:raise RuntimeError("2103169 edge-to-edge contract missing "+token)
    for token in (
        ...
        if token not in surface:raise RuntimeError("2103169 surface resolver missing "+token)
    ...
    proof={
        ...
        "new_helpers":["cobraConfigureEdgeToEdgeSystemBars","cobraBrowseSystemBarSurfaceColor"],
        "protected_methods":protected_before,
        "root_cause":"visible status-bar region was not guaranteed to be inside Cobra's drawable edge-to-edge window, leaving Samsung's black system surface exposed",
        "browse_status_bar":"visible transparent status bar over an explicitly edge-to-edge theme-matched safe-area band",
    ...
    print("PASS: locked 2103167 -> 2103169 edge-to-edge status-bar repair; safe-area/player/PiP untouched")
