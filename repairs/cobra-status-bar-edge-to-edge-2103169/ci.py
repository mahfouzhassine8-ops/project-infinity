#!/usr/bin/env python3
"""Locked 2103167 -> isolated 2103169 status-bar edge-to-edge gates."""
...
def upgrade():
    ...
    out=Path('audit169');out.mkdir(exist_ok=True)
    ...
    replace('kodi/tools/android/packaging/xbmc/build.gradle.in','versionCode 2103167','versionCode 2103169')
    replace('kodi/tools/android/packaging/xbmc/build.gradle.in','versionName "'+OLD+'"','versionName "'+NEW+'"')
    replace('scripts/infinity_background_resume.py','VERSION_CODE = 2103167','VERSION_CODE = 2103169')
    ...
    print('PASS: exact locked 2103167 -> isolated 2103169 status-bar surface delta')

def verify():
    baseline_identity()
    apk=Path('signed169/Infinity-'+NEW+'.apk')
    audit=json.loads(Path('signed169/background-resume-apk-audit.json').read_text())
    ...
    manifest=Path('signed169/manifest.txt').read_text()
    ...
    Path('audit169/apk-verification.json').write_text(json.dumps(proof,indent=2)+'\n')
...
def deliver():
    ...
    for name,count in [('CobraVisualRuntimeTest',17),('CobraVisualLayoutTest',10)]:
        inherited+=suite(Path('audit169/android/runtime/test-results')/('TEST-com.projectinfinity.kodi.'+name+'.xml'),count)

    candidate14=suite('audit169/targeted/test-results/TEST-com.projectinfinity.kodi.Cobra2103162ThemeRotationTest.xml',16)
    locked164=suite('audit169/targeted/test-results/TEST-com.projectinfinity.kodi.Cobra2103164PlaybackStabilityTest.xml',26)
    ...
    patch=json.loads(Path('audit169/patch/patch.json').read_text())
    require(patch.get('changed_methods')==['cobraApplySystemBarsForSurface','cobraConfirmBrowseSystemBars'],'Scope drift in 2103169')
    require(patch.get('new_helpers')==['cobraConfigureEdgeToEdgeSystemBars','cobraBrowseSystemBarSurfaceColor'],'Helper contract drift in 2103169')
    require(patch.get('layout_changed') is False,'Layout changed in status-bar micro-fix')
    require(patch.get('safe_area_changed') is False,'Safe-area geometry changed in 2103169')
    require(patch.get('window_fit_changed') is True and patch.get('decor_fits_system_windows') is False,'Edge-to-edge window-fit contract missing in 2103169')
    require(patch.get('player_changed') is False,'Player changed in 2103169')

    apkproof=json.loads(Path('audit169/apk-verification.json').read_text())
    ...
    report={
        ...
        'locked_2103166_tests':locked166,'locked_2103167_tests':locked167,'new_2103169_tests':new,'total_android_tests':total,
        ...
        'status':'TEST CANDIDATE - physical Fold black-strip removal confirmation required'
    }
    Path('signed169/ACCEPTANCE.json').write_text(json.dumps(report,indent=2)+'\n')
    shutil.copy2('audit169/apk-verification.json','signed169/2103169-verification.json')
    shutil.copy2('audit169/source-audit/source-audit.json','signed169/2103169-source-audit.json')
    shutil.copy2('audit169/patch/patch.json','signed169/2103169-source-proof.json')
    shutil.copy2(ROOT/'AUDIT.md','signed169/AUDIT-2103169.md')
