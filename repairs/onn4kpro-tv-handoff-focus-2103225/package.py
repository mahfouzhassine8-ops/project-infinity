#!/usr/bin/env python3
"""Fast-pack the audited 2103225 RC8 Android TV layer over exact signed locked 2103224 RC7 ARMv7 APK.

No Kodi native rebuild, no apktool and no smali. Java/manifest/resources are
compiled from source, resource IDs are pinned to the protected 2103224 RC7 APK, and
only compiled DEX + manifest replace their counterparts. All native libraries,
assets and compiled Android resources remain byte-identical to 2103224.
"""
...
VERSION=2103225
RELEASE='1.0.9-Cobra-Onn4KPro-TV-Handoff-Focus-RC8'
BASE_APK_SHA256='65ba5d48a9070eaadcaa564d3e80d5850aac96c2c6a02f57b149f5f5a424a994'
...
def source_preservation(source:Path):
    ...
    require(receipt.get('version_code')==VERSION and receipt.get('version_name')==RELEASE,'Wrong audited 2103225 source receipt')
...
def prepare(source:Path,base:Path,build:Path,out:Path):
    require(sha(base.read_bytes())==BASE_APK_SHA256,'Not exact locked 2103224 RC7 signed TV APK')
...
def verify_bytes(base:Path,final:Path):
    with zipfile.ZipFile(base) as a,zipfile.ZipFile(final) as b:
        ...
        for token in (b'cobra_tv_grid_only_2103220',b'cobra_tv_remote_ui_2103221',b'cobra_tv_player_multiview_2103222',b'cobra_tv_hardening_2103223',
                      b'Return to Multi-View',b'Search and add',b'Enlarge screen',b'cobra_tv_player_tool_channels',b'Opening saved TV library',b'TV SYSTEM',b'cobra_tv_native_remote_2103224',b'cobra_tv_handoff_focus_2103225',b'cobra_multi_filter:',b'Channel group ',b'Select to change group.',b'onRenderedFirstFrame'):
            require(token in joined,'Missing 2103225 runtime marker: '+repr(token))
...
def main():
    ...
    unsigned=out/'Infinity-1.0.9-Cobra-Onn4KPro-TV-Handoff-Focus-RC8-unsigned.apk'
    ...
    final=out/'Infinity-1.0.9-Cobra-Onn4KPro-TV-Handoff-Focus-RC8.apk';run('bash',ROOT/'scripts/sign-infinity71.sh',unsigned,final)
    ...
    report={'schema':1,'build':VERSION,'version_name':RELEASE,'package':PACKAGE,'target_abi':'armeabi-v7a','hardening':True,'native_remote_architecture':True,'handoff_first_render':True,'multiview_focus_visible':True,'player_footer_unclipped':True,'multiview_two_to_one_session_preserved':True,
            'base_build':2103224,'base_apk_sha256':BASE_APK_SHA256,'base_native_engine_sha256':BASE_ENGINE_SHA256,
    ...
    (out/'ONN-TV-2103225-VERIFICATION.json').write_text(json.dumps(report,indent=2,sort_keys=True)+'\n')
    shutil.copy2(ROOT/'engine/background-resume-source.json',out/'background-resume-source.json')
    shutil.copy2(ROOT/'audit225/tv-handoff-focus-source.json',out/'tv-handoff-focus-source.json')
    unsigned.unlink()
    print('PASS: 2103225 RC8 TV Android layer + exact locked 2103224 RC7 APK payload/native/resources; permanent signer verified')
