#!/usr/bin/env python3
"""Package/gate 2103174 playback buffer regression over exact locked 2103171."""
...
NEW="1.0.9-Cobra-Playback-Buffer-Regression-RC1"
VERSION=2103174
...
def upgrade():
 ...
 run("python3",ROOT/"apply.py","--source","kodi","--receipt",receipt,"--out","audit174/patch")
 patch=json.loads(Path("audit174/patch/patch.json").read_text())
 ...
 data.update(
   ...
   pre_2103153_single_view_buffer_restored=True,
 ...
 for rel,row in data["files"].items():
  require(sha(Path("kodi")/rel)==row["after"],"Final 2103174 source receipt drift: "+rel)

 run("python3",ROOT/"source_audit.py","--source","kodi","--patch","audit174/patch/patch.json","--out","audit174/source-audit")

 with Path("audit174/native-after.patch").open("wb") as out:
  subprocess.run(["git","-C","kodi","diff","--binary","--","xbmc"],stdout=out,check=True)
 require(Path("audit174/native-after.patch").read_bytes()==Path("engine/native-before.patch").read_bytes(),"Native source changed")
 print("PASS: exact 2103171 -> 2103174 playback buffer regression; native tree unchanged")

def verify():
 base=Path("baseline171/Infinity-"+OLD+".apk")
 final=Path("signed174/Infinity-"+NEW+".apk")
 ...
 audit=json.loads(Path("signed174/background-resume-apk-audit.json").read_text())
 require(audit["version_code"]==VERSION and audit["version_name"]==NEW,"Final 2103174 identity mismatch")
 ...
 with zipfile.ZipFile(base) as old,zipfile.ZipFile(final) as new:
  ...
  for token in (b"buffer_wait_network",b"same_session=true",b"CobraBufferRegressionProfile",
 ...
 result={
  ...
  "physical_device_verified":False}
 Path("audit174/final-verification.json").write_text(json.dumps(result,indent=2)+"\n")
 print("PASS: signed 2103174 preserves exact 2103171 native/resources and release signer")
...
def deliver():
 ...
 inherited+=suite("audit174/android/runtime/test-results/TEST-com.projectinfinity.kodi.CobraVisualRuntimeTest.xml",17)
 inherited+=suite("audit174/android/runtime/test-results/TEST-com.projectinfinity.kodi.CobraVisualLayoutTest.xml",10)
 targeted=0
 for name,count in [
 ...
 source=json.loads(Path("audit174/source-audit/source-audit.json").read_text())
 final=json.loads(Path("audit174/final-verification.json").read_text())
 ...
 result={
  ...
  "android_tests":total,"new_buffering_tests":3,
  "watchdog_reprepare_removed":True,
  "pre_2103153_single_view_load_control_restored":True,
  "single_view_headroom":"Media3 DefaultLoadControl restored (50s min/max default streaming buffer)",
 ...
 Path("signed174/ACCEPTANCE.json").write_text(json.dumps(result,indent=2)+"\n")
 shutil.copy2("audit174/final-verification.json","signed174/2103174-verification.json")
 shutil.copy2("audit174/source-audit/source-audit.json","signed174/2103174-source-audit.json")
