#!/usr/bin/env python3
"""Package/gate 2103193 source manager restore over exact passed 2103192."""
...
OLD='1.0.9-Cobra-Provider-Pace-Guard-RC1'
NEW='1.0.9-Cobra-Source-Manager-Restore-RC1'
VERSION=2103193
...
def upgrade():
    ...
    require(data.get('version_code')==2103192 and data.get('version_name')==OLD,'Expected exact passed 2103192 source receipt')
    for rel,row in data.get('files',{}).items():
        require(sha(Path('kodi')/rel)==row['after'],'2103192 source receipt drift: '+rel)

    run('python3',ROOT/'apply.py','--source','kodi','--receipt',receipt,'--out','audit193/patch')
    patch=json.loads(Path('audit193/patch/patch.json').read_text());data['files'][ACT]['after']=patch['files'][ACT]['after']

    gradle=Path('kodi')/GRADLE
    replace(gradle,'versionCode 2103192',f'versionCode {VERSION}')
    replace(gradle,'versionName "'+OLD+'"','versionName "'+NEW+'"')
    runtime=Path('scripts/infinity_background_resume.py')
    replace(runtime,'VERSION_CODE = 2103192',f'VERSION_CODE = {VERSION}')
    ...
    require(s.count(needle)>=2,'2103192 packager identity drift');pack.write_text(s.replace(needle,'Infinity-'+NEW))

    data['files'][GRADLE]['after']=sha(gradle)
    data.update(
      version_code=VERSION,version_name=NEW,source_parent=2103192,source_parent_locked=True,locked_parent_commit=BASE_COMMIT,candidate_locked=False,
      ...
      clock_rewrite_suppressed_by_evidence=True,source_manager_route_restored=True,source_manager_label='TV SOURCES',
    ...
    for rel,row in data['files'].items():
        require(sha(Path('kodi')/rel)==row['after'],'Final 2103193 source receipt drift: '+rel)
    run('python3',ROOT/'source_audit.py','--source','kodi','--patch','audit193/patch/patch.json','--out','audit193/source-audit')
    with Path('audit193/native-after.patch').open('wb') as out:
        subprocess.run(['git','-C','kodi','diff','--binary','--','xbmc'],stdout=out,check=True)
    require(Path('audit193/native-after.patch').read_bytes()==Path('engine/native-before.patch').read_bytes(),'Native source changed')
    print('PASS: exact 2103192 -> 2103193 source manager restore; protected stack preserved')

def verify():
    base=Path('baseline192/Infinity-'+OLD+'.apk')
    final=Path('signed193/Infinity-'+NEW+'.apk')
    require(base.is_file() and final.is_file(),'Baseline/final APK missing')
    bv=json.loads(Path('baseline192/2103192-verification.json').read_text())
    require(bv['apk_sha256']==sha(base),'Wrong exact passed 2103192 APK')
    audit=json.loads(Path('signed193/background-resume-apk-audit.json').read_text())
    require(audit['version_code']==VERSION and audit['version_name']==NEW,'Final 2103193 identity mismatch')
    ...
    result={'build':VERSION,'base_build':2103192,'base_commit':BASE_COMMIT,'base_apk_sha256':sha(base),'apk_sha256':sha(final),'signer':CERT,
      'stream_fingerprint_schema':7,'provider_route_fingerprint':True,'provider_pace_limit_classifier':True,'timeline_normalizer_rewrite_enabled':False,
      'clock_rewrite_suppressed_by_evidence':True,'source_manager_route_restored':True,'source_manager_label':'TV SOURCES',
      'playback_behavior_changed':False,'network_selection_changed':False,'timeshift_ownership_changed':False,'buffer_policy_changed':False,
      'parser_flags_changed':False,'native_engine_recompiled':False,'theme_zip_changed':False,'physical_device_verified':False}
    Path('audit193/final-verification.json').write_text(json.dumps(result,indent=2)+'\n')
    print('PASS: signed 2103193 preserves exact 2103192 native/resources/signer and restores source-manager navigation only')
...
def deliver():
    ...
    total+=suite('audit193/android/runtime/test-results/TEST-com.projectinfinity.kodi.CobraVisualRuntimeTest.xml',17)
    total+=suite('audit193/android/runtime/test-results/TEST-com.projectinfinity.kodi.CobraVisualLayoutTest.xml',10)
    for name,count in [
      ...
      ('Cobra2103190TsClockNormalizationTest',5),('Cobra2103191ProviderRouteFingerprintTest',5),('Cobra2103192ProviderPaceReserveTest',5),('Cobra2103193SourceManagerRestoreTest',5)]:
        total+=suite(Path('audit193/targeted/test-results')/('TEST-com.projectinfinity.kodi.'+name+'.xml'),count)
    require(total==237,f'Expected 237 Android tests, got {total}')
    source=json.loads(Path('audit193/source-audit/source-audit.json').read_text())
    final=json.loads(Path('audit193/final-verification.json').read_text())
    require(source.get('passed') and not source.get('failed'),'2103193 source audit failed')
    result={'build':VERSION,'locked_parent':2103192,'locked_parent_commit':BASE_COMMIT,'android_tests':total,'new_source_manager_tests':5,
      'stream_fingerprint_schema':7,'provider_route_fingerprint':True,'provider_pace_limit_classifier':True,'source_manager_route_restored':True,
      ...
      'status':'TEST CANDIDATE - Cobra Settings must expose TV SOURCES and open the existing source manager; source/provider/playback logic remains unchanged'}
    Path('signed193/ACCEPTANCE.json').write_text(json.dumps(result,indent=2)+'\n')
    shutil.copy2('audit193/final-verification.json','signed193/2103193-verification.json')
    shutil.copy2('audit193/source-audit/source-audit.json','signed193/2103193-source-audit.json')
    print('PASS:',total,'Android tests + 2103193 source-manager restore gates')
