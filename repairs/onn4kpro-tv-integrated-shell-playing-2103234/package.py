#!/usr/bin/env python3
"""Fast-pack the audited 2103234 RC14 integrated-shell layer over exact signed locked 2103233 RC13 ARMv7 APK.

No Kodi native rebuild, no apktool and no smali. Java/manifest/resources are
compiled from source, resource IDs are pinned to the protected 2103233 RC13 APK, and
only compiled DEX + manifest replace their counterparts. All native libraries,
assets and compiled Android resources remain byte-identical to 2103233.
"""
...
VERSION=2103234
RELEASE='1.0.9-Cobra-Onn4KPro-TV-Integrated-Shell-Playing-RC14'
BASE_APK_SHA256='7dd995e834227db31e66469450b65079100efcfe5ae4283b95984ac44b16959a'
...
def source_preservation(source:Path):
    ...
    require(receipt.get('version_code')==VERSION and receipt.get('version_name')==RELEASE,'Wrong audited 2103234 source receipt')
...
def prepare(source:Path,base:Path,build:Path,out:Path):
    require(sha(base.read_bytes())==BASE_APK_SHA256,'Not exact locked 2103233 RC13 signed TV APK')
...
def main():
    ...
    unsigned=out/'Infinity-1.0.9-Cobra-Onn4KPro-TV-Integrated-Shell-Playing-RC14-unsigned.apk'
    ...
    final=out/'Infinity-1.0.9-Cobra-Onn4KPro-TV-Integrated-Shell-Playing-RC14.apk';run('bash',ROOT/'scripts/sign-infinity71.sh',unsigned,final)
    ...
    (out/'ONN-TV-2103234-VERIFICATION.json').write_text(json.dumps(report,indent=2,sort_keys=True)+'\n')
    shutil.copy2(ROOT/'engine/background-resume-source.json',out/'background-resume-source.json')
    shutil.copy2(ROOT/'audit234/tv-integrated-shell-playing-source.json',out/'tv-integrated-shell-playing-source.json')
    unsigned.unlink()
    print('PASS: 2103234 RC14 integrated shell TV layer + exact locked 2103233 RC13 APK payload/native/resources; permanent signer verified')
