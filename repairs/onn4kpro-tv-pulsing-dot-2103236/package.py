#!/usr/bin/env python3
"""Fast-pack the audited 2103236 RC16 pulsing-dot layer over exact signed locked 2103235 RC15 ARMv7 APK.

No Kodi native rebuild, no apktool and no smali. Java/manifest/resources are
compiled from source, resource IDs are pinned to the protected 2103235 RC15 APK, and
only compiled DEX + manifest replace their counterparts. All native libraries,
assets and compiled Android resources remain byte-identical to 2103235.
"""
...
VERSION=2103236
RELEASE='1.0.9-Cobra-Onn4KPro-TV-Pulsing-Dot-RC16'
BASE_APK_SHA256='bd864a19078c18fa63aeda4b90ac81eaa4a59c478e799e9de5791aa8eb09498e'
...
def source_preservation(source:Path):
    ...
    require(receipt.get('version_code')==VERSION and receipt.get('version_name')==RELEASE,'Wrong audited 2103236 source receipt')
...
def prepare(source:Path,base:Path,build:Path,out:Path):
    require(sha(base.read_bytes())==BASE_APK_SHA256,'Not exact locked 2103235 RC15 signed TV APK')
...
def verify_bytes(base:Path,final:Path):
    with zipfile.ZipFile(base) as a,zipfile.ZipFile(final) as b:
        ...
        for token in (b'cobra_tv_grid_only_2103220',b'cobra_tv_remote_ui_2103221',b'cobra_tv_player_multiview_2103222',b'cobra_tv_hardening_2103223',
                      b'Return to Multi-View',b'Search and add',b'Enlarge screen',b'cobra_tv_player_tool_channels',b'Opening saved TV library',b'TV SYSTEM',b'cobra_tv_native_remote_2103224',b'cobra_tv_handoff_focus_2103225',b'cobra_tv_navigation_motion_2103230',b'cobra_tv_adaptive_ui_2103231',b'cobra_tv_integrated_drawer_2103232',b'cobra_tv_glass_system_2103233',b'cobra_tv_integrated_shell_playing_2103234',b'cobra_tv_final_polish_2103235',b'CobraTvPlayingDot',b'NORMAL_PULSE_MS',b'CINEMA_PULSE_MS',b'0xffffc247',b'cobra_movies_search',b'cobra_shows_search',b'Search TV Shows',b'Search Movies',b'Lightweight Glass UI is always active.',b'PLAYING',b'Back to guide',b'cobra_tv_grid_settings',b'cobraTvShowGroupChooser',b'cobraTvSelectGroup',b'cobra_multi_filter:',b'onRenderedFirstFrame'):
            require(token in joined,'Missing 2103236 runtime marker: '+repr(token))
...
def main():
    ...
    unsigned=out/'Infinity-1.0.9-Cobra-Onn4KPro-TV-Pulsing-Dot-RC16-unsigned.apk'
    ...
    final=out/'Infinity-1.0.9-Cobra-Onn4KPro-TV-Pulsing-Dot-RC16.apk';run('bash',ROOT/'scripts/sign-infinity71.sh',unsigned,final)
    ...
    (out/'ONN-TV-2103236-VERIFICATION.json').write_text(json.dumps(report,indent=2,sort_keys=True)+'\n')
    shutil.copy2(ROOT/'engine/background-resume-source.json',out/'background-resume-source.json')
    shutil.copy2(ROOT/'audit236/tv-pulsing-dot-source.json',out/'tv-pulsing-dot-source.json')
    unsigned.unlink()
    print('PASS: 2103236 RC16 pulsing-dot TV layer + exact locked 2103235 RC15 APK payload/native/resources; permanent signer verified')
