#!/usr/bin/env python3
"""Fast-pack the audited 2103230 RC10 TV navigation layer over exact signed locked 2103225 RC8 ARMv7 APK.
...
VERSION=2103230
RELEASE='1.0.9-Cobra-Onn4KPro-TV-Navigation-Motion-RC10'
...
def source_preservation(source:Path):
    ...
    require(receipt.get('version_code')==VERSION and receipt.get('version_name')==RELEASE,'Wrong audited 2103230 source receipt')
...
def verify_bytes(base:Path,final:Path):
    with zipfile.ZipFile(base) as a,zipfile.ZipFile(final) as b:
        ...
        for token in (b'cobra_tv_grid_only_2103220',b'cobra_tv_remote_ui_2103221',b'cobra_tv_player_multiview_2103222',b'cobra_tv_hardening_2103223',
                      b'Return to Multi-View',b'Search and add',b'Enlarge screen',b'cobra_tv_player_tool_channels',b'Opening saved TV library',b'TV SYSTEM',b'cobra_tv_native_remote_2103224',b'cobra_tv_handoff_focus_2103225',b'cobra_tv_navigation_motion_2103230',b'Back to guide',b'cobra_multi_filter:',b'Channel group ',b'Select to change group.',b'onRenderedFirstFrame'):
            require(token in joined,'Missing 2103230 runtime marker: '+repr(token))
...
def main():
    ...
    unsigned=out/'Infinity-1.0.9-Cobra-Onn4KPro-TV-Navigation-Motion-RC10-unsigned.apk'
    ...
    final=out/'Infinity-1.0.9-Cobra-Onn4KPro-TV-Navigation-Motion-RC10.apk';run('bash',ROOT/'scripts/sign-infinity71.sh',unsigned,final)
    ...
    (out/'ONN-TV-2103230-VERIFICATION.json').write_text(json.dumps(report,indent=2,sort_keys=True)+'\n')
    shutil.copy2(ROOT/'engine/background-resume-source.json',out/'background-resume-source.json')
    shutil.copy2(ROOT/'audit230/tv-navigation-motion-source.json',out/'tv-navigation-motion-source.json')
    unsigned.unlink()
    print('PASS: 2103230 RC10 navigation TV layer + exact locked 2103225 RC8 APK payload/native/resources; permanent signer verified')
