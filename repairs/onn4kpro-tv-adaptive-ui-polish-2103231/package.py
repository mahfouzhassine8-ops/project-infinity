#!/usr/bin/env python3
"""Fast-pack the audited 2103231 RC11 adaptive UI layer over exact signed locked 2103230 RC10 ARMv7 APK.

No Kodi native rebuild, no apktool and no smali. Java/manifest/resources are
compiled from source, resource IDs are pinned to the protected 2103230 RC10 APK, and
only compiled DEX + manifest replace their counterparts. All native libraries,
assets and compiled Android resources remain byte-identical to 2103230.
"""
...
VERSION=2103231
RELEASE='1.0.9-Cobra-Onn4KPro-TV-Adaptive-UI-Polish-RC11'
BASE_APK_SHA256='c0f43132993ad0dce094b442d25f0b797228498b821a9cadbd0c6c2f47ccdc88'
...
def source_preservation(source:Path):
    ...
    require(receipt.get('version_code')==VERSION and receipt.get('version_name')==RELEASE,'Wrong audited 2103231 source receipt')
...
def prepare(source:Path,base:Path,build:Path,out:Path):
    require(sha(base.read_bytes())==BASE_APK_SHA256,'Not exact locked 2103230 RC10 signed TV APK')
...
def main():
    ...
    unsigned=out/'Infinity-1.0.9-Cobra-Onn4KPro-TV-Adaptive-UI-Polish-RC11-unsigned.apk'
    ...
    final=out/'Infinity-1.0.9-Cobra-Onn4KPro-TV-Adaptive-UI-Polish-RC11.apk';run('bash',ROOT/'scripts/sign-infinity71.sh',unsigned,final)
    ...
    report={'schema':1,'build':VERSION,'version_name':RELEASE,'package':PACKAGE,'target_abi':'armeabi-v7a','hardening':True,'native_remote_architecture':True,'handoff_first_render':True,'multiview_focus_visible':True,'player_footer_unclipped':True,'multiview_two_to_one_session_preserved':True,'navigation_hierarchy':'drawer-groups-grid-player','back_reverses_hierarchy':True,'mini_player_preserved':True,'preview_engine_preserved':True,'player_channels_control_preserved':True,'group_counts_cached':True,'hamburger_removed':True,'grid_group_dropdown_removed':True,'settings_next_to_search':True,'mini_player_adaptive_large':True,
            'base_build':2103230,'base_apk_sha256':BASE_APK_SHA256,'base_native_engine_sha256':BASE_ENGINE_SHA256,
    ...
    (out/'ONN-TV-2103231-VERIFICATION.json').write_text(json.dumps(report,indent=2,sort_keys=True)+'\n')
    shutil.copy2(ROOT/'engine/background-resume-source.json',out/'background-resume-source.json')
    shutil.copy2(ROOT/'audit231/tv-adaptive-ui-source.json',out/'tv-adaptive-ui-source.json')
    unsigned.unlink()
    print('PASS: 2103231 RC11 adaptive UI TV layer + exact locked 2103230 RC10 APK payload/native/resources; permanent signer verified')
