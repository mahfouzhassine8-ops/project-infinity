#!/usr/bin/env python3
"""Package/gate 2103196 original player-settings restore over exact passed 2103194 while keeping Player options + Recent Channels."""
...
NEW='1.0.9-Cobra-Original-Player-Settings-RC1'
VERSION=2103196
...
def upgrade():
    ...
    run('python3',ROOT/'apply.py','--source','kodi','--receipt',receipt,'--out','audit196/patch')
    patch=json.loads(Path('audit196/patch/patch.json').read_text());data['files'][ACT]['after']=patch['files'][ACT]['after']
    ...
    for rel,row in data['files'].items():
        require(sha(Path('kodi')/rel)==row['after'],'Final 2103196 source receipt drift: '+rel)
    run('python3',ROOT/'source_audit.py','--source','kodi','--patch','audit196/patch/patch.json','--out','audit196/source-audit')
    with Path('audit196/native-after.patch').open('wb') as out:
        subprocess.run(['git','-C','kodi','diff','--binary','--','xbmc'],stdout=out,check=True)
    require(Path('audit196/native-after.patch').read_bytes()==Path('engine/native-before.patch').read_bytes(),'Native source changed')
    print('PASS: exact 2103194 -> 2103196 original player settings restore; protected stack preserved')

def verify():
    base=Path('baseline194/Infinity-'+OLD+'.apk')
    final=Path('signed196/Infinity-'+NEW+'.apk')
    ...
    audit=json.loads(Path('signed196/background-resume-apk-audit.json').read_text())
    require(audit['version_code']==VERSION and audit['version_name']==NEW,'Final 2103196 identity mismatch')
    ...
    with zipfile.ZipFile(base) as old,zipfile.ZipFile(final) as new:
        ...
        for token in (
          ...
          b'cobra_unified_live_timeline',b'CobraCallAudioPolicy',b'buffer_observed_no_restart',b'TV SOURCES',b'cobra_tv_sources',b'Fold Adaptive',b'Player options',b'RECENT CHANNELS',b'Channel playback',b'Aspect / Display',b'Manage sources',b'Close player',b'CobraMotionSpec',b'cobra-default-aspect:'):
            require(token in dex,'2103196 contract missing: '+repr(token))
        require(b'Cobra2103196OriginalPlayerSettingsTest' not in dex,'2103196 test code packaged in release APK')

    badging=Path('signed196/badging.txt').read_text()
    require("package: name='com.projectinfinity.kodi'" in badging and "versionCode='2103196'" in badging,'Final APK not forward-installable')
    result={'build':VERSION,'base_build':2103194,'base_commit':BASE_COMMIT,'base_apk_sha256':sha(base),'apk_sha256':sha(final),'signer':CERT,
      ...
      'playback_behavior_changed':False,'network_selection_changed':False,'timeshift_ownership_changed':False,'buffer_policy_changed':False,
      'parser_flags_changed':False,'native_engine_recompiled':False,'theme_zip_changed':False,'physical_device_verified':False}
    Path('audit196/final-verification.json').write_text(json.dumps(result,indent=2)+'\n')
    print('PASS: signed 2103196 preserves exact 2103194 native/resources/signer and keeps Player options + Recent Channels while restoring original player settings only')
...
def deliver():
    ...
    total+=suite('audit196/android/runtime/test-results/TEST-com.projectinfinity.kodi.CobraVisualRuntimeTest.xml',17)
    total+=suite('audit196/android/runtime/test-results/TEST-com.projectinfinity.kodi.CobraVisualLayoutTest.xml',10)
    for name,count in [
      ...
      ('Cobra2103190TsClockNormalizationTest',5),('Cobra2103191ProviderRouteFingerprintTest',5),('Cobra2103192ProviderPaceReserveTest',5),('Cobra2103193SourceManagerRestoreTest',5),('Cobra2103194FoldAdaptiveAspectTest',6),('Cobra2103196OriginalPlayerSettingsTest',10)]:
        total+=suite(Path('audit196/targeted/test-results')/('TEST-com.projectinfinity.kodi.'+name+'.xml'),count)
    require(total==253,f'Expected 253 Android tests, got {total}')
    source=json.loads(Path('audit196/source-audit/source-audit.json').read_text())
    final=json.loads(Path('audit196/final-verification.json').read_text())
    require(source.get('passed') and not source.get('failed'),'2103196 source audit failed')
    result={'build':VERSION,'locked_parent':2103194,'locked_parent_commit':BASE_COMMIT,'android_tests':total,'new_original_player_settings_tests':10,
      ...
      'status':'TEST CANDIDATE - Player options + Recent Channels retained with exact pre-2103195 player settings body restored; playback/network/timeshift/native contracts remain protected'}
    Path('signed196/ACCEPTANCE.json').write_text(json.dumps(result,indent=2)+'\n')
    shutil.copy2('audit196/final-verification.json','signed196/2103196-verification.json')
    shutil.copy2('audit196/source-audit/source-audit.json','signed196/2103196-source-audit.json')
    print('PASS:',total,'Android tests + 2103196 original player-settings restore gates')
