...
def main():
    ...
    hub=block(text,'showCobraPlayerOptionsHub');video=block(text,'showCobraVideoOptions');chrome=block(text,'cobraBuildPlayerChrome');player_settings=block(text,'showPlayerSettingsDrawer')
    ...
    checks={
      ...
      'player_hub':'showPlayerSettingsDrawer();' in hub,
      'recent_strip':'cobraAddRecentPlayerStrip(rows)' in player_settings and 'RECENT CHANNELS' in block(text,'cobraAddRecentPlayerStrip') and 'cobra-player-recent:' in text,
      ...
      'original_player_settings_restored':patch.get('original_player_settings_restored') is True and patch.get('player_settings_body_preserved') is True and 'Player options' in player_settings and 'cobraAddRecentPlayerStrip(rows)' in player_settings,
    ...
    result={'passed':not failed,'checks':checks,'failed':failed,'build':2103196,'original_player_settings_restored':True,'physical_device_verified':False}
    a.out.mkdir(parents=True,exist_ok=True);(a.out/'source-audit.json').write_text(json.dumps(result,indent=2)+'\n')
    if failed: raise RuntimeError('2103196 source audit failed: '+', '.join(failed))
    print('PASS:',len(checks),'2103196 source checks')
