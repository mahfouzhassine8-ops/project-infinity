...
def apply(source,receipt_path,out):
    ...
    for token in required:
        if token not in text: raise RuntimeError('2103196 contract missing: '+token)
    ...
    report={
      ...
      'tivimate_inspired_player_hub':True,'recent_channel_strip':True,'video_display_submenu':True,'original_player_settings_restored':True,'player_settings_body_preserved':True,
    ...
    print('PASS: 2103196 keeps Player options + Recent Channels while restoring the exact pre-2103195 player settings body over 2103194; protected playback/source stack preserved')
