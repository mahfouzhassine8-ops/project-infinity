#!/usr/bin/env python3
"""Fast-pack the audited 2103226 RC9 lightweight TV layer over exact signed locked 2103225 RC8 ARMv7 APK.

No Kodi native rebuild, no apktool and no smali. Java/manifest/resources are
compiled from source, resource IDs are pinned to the protected 2103225 RC8 APK, and
only compiled DEX + manifest replace their counterparts. All native libraries,
assets and compiled Android resources remain byte-identical to 2103225.
"""
...
VERSION=2103226
RELEASE='1.0.9-Cobra-Onn4KPro-TV-Lightweight-Live-RC9'
BASE_APK_SHA256='b4e97e83a40eeb6db02fe491be842427ac6c0a8c6254ec0648300b4e23e6e8e6'
...
def source_preservation(source:Path):
    ...
    require(receipt.get('version_code')==VERSION and receipt.get('version_name')==RELEASE,'Wrong audited 2103226 source receipt')
    ...
    require(receipt.get('tv_lightweight_live_architecture') is True and receipt.get('tv_persistent_player_surface') is True,'2103226 lightweight Live TV receipt missing')
    require(receipt.get('tv_guide_preview_decoder_disabled') is True and receipt.get('tv_channel_library_index_cached') is True,'2103226 lightweight resource gates missing')
    require(receipt.get('tv_channel_group_counts_cached') is True and receipt.get('tv_player_surface_not_resized_by_overlays') is True,'2103226 TV overlay/index gates missing')
...
def prepare(source:Path,base:Path,build:Path,out:Path):
    require(sha(base.read_bytes())==BASE_APK_SHA256,'Not exact locked 2103225 RC8 signed TV APK')
...
def verify_bytes(base:Path,final:Path):
    with zipfile.ZipFile(base) as a,zipfile.ZipFile(final) as b:
        ...
        for token in (b'cobra_tv_grid_only_2103220',b'cobra_tv_remote_ui_2103221',b'cobra_tv_player_multiview_2103222',b'cobra_tv_hardening_2103223',
                      b'Return to Multi-View',b'Search and add',b'Enlarge screen',b'cobra_tv_player_tool_channels',b'Opening saved TV library',b'TV SYSTEM',b'cobra_tv_native_remote_2103224',b'cobra_tv_handoff_focus_2103225',b'cobra_tv_lightweight_live_2103226',b'Browse channels',b'cobra_multi_filter:',b'Channel group ',b'Select to change group.',b'onRenderedFirstFrame'):
            require(token in joined,'Missing 2103226 runtime marker: '+repr(token))
...
def main():
    ...
    unsigned=out/'Infinity-1.0.9-Cobra-Onn4KPro-TV-Lightweight-Live-RC9-unsigned.apk'
    ...
    final=out/'Infinity-1.0.9-Cobra-Onn4KPro-TV-Lightweight-Live-RC9.apk';run('bash',ROOT/'scripts/sign-infinity71.sh',unsigned,final)
    ...
    report={'schema':1,'build':VERSION,'version_name':RELEASE,'package':PACKAGE,'target_abi':'armeabi-v7a','hardening':True,'native_remote_architecture':True,'handoff_first_render':True,'multiview_focus_visible':True,'player_footer_unclipped':True,'multiview_two_to_one_session_preserved':True,'lightweight_live_architecture':True,'persistent_player_surface':True,'guide_preview_decoder_disabled':True,'channel_library_index_cached':True,'channel_group_counts_cached':True,'player_surface_not_resized_by_overlays':True,
            'base_build':2103225,'base_apk_sha256':BASE_APK_SHA256,'base_native_engine_sha256':BASE_ENGINE_SHA256,
    ...
    (out/'ONN-TV-2103226-VERIFICATION.json').write_text(json.dumps(report,indent=2,sort_keys=True)+'\n')
    shutil.copy2(ROOT/'engine/background-resume-source.json',out/'background-resume-source.json')
    shutil.copy2(ROOT/'audit226/tv-lightweight-live-source.json',out/'tv-lightweight-live-source.json')
    unsigned.unlink()
    print('PASS: 2103226 RC9 lightweight TV Android layer + exact locked 2103225 RC8 APK payload/native/resources; permanent signer verified')
