#!/usr/bin/env python3
"""Exact locked 2103158 -> 2103159 experience bridge, scoped Cobra cleanup, rollback and delivery gates."""
...
def reconstruct():
 ...
 (rollback/'README.txt').write_text('Exact locked 2103158 APK/UI/source before the experience ZIP bridge and Cobra presentation cleanup. Userdata is NOT included. Do not uninstall or clear data to force a downgrade; Android can reject a lower versionCode.\n')
...
def promote():
 patch=json.loads(Path('audit159/patch.json').read_text());require(sha(Path('kodi')/SPLASH)==patch['after_sha256'],'Splash differs from tested bridge');require(patch['activity_before_sha256']==ACTIVITY158,'Wrong Cobra cleanup parent');require(sha(Path('kodi')/ACT)==patch['activity_after_sha256'],'Cobra Activity differs from audited cleanup')
 pairs={
 ...
 data.update(version_code=2103159,version_name=NEW,locked_parent=2103158,source_parent=2103158,source_parent_locked=True,locked_commit=LOCKED,candidate_locked=False,experience_theme_bridge=1,experience_ui_zip='1.4.0',cobra_navigation_cleanup=1,physical_device_verified=False,native_engine_unchanged=True)
 ...
 for name,row in original['files'].items():
  if name not in ('tools/android/packaging/xbmc/build.gradle.in',ACT):require(sha(Path('kodi')/name)==row['after'],'Locked 2103158 source changed: '+name)
 require(sha(Path('kodi')/ACT)==patch['activity_after_sha256'],'Scoped Cobra cleanup changed during promotion')
 print('PASS: 2103159 identity promoted; Splash bridge + scoped Cobra navigation cleanup; native/health/playback contracts preserved')
def verify():
 apk=Path('signed159/Infinity-'+NEW+'.apk');ui=Path('signed159/Infinity-Cobra-UI-1.4.0-Experience.zip');patch=json.loads(Path('audit159/patch.json').read_text())
 with zipfile.ZipFile(apk) as new,zipfile.ZipFile(baseline_apk()) as old:
  ...
  for token in (b'infinity-experience-chooser',b'experience-themed-root',b'experience-initials',b'INFINITY BY HASSINE MAHFOUZ',b'YOUR HOME FOR MOVIES, SHOWS AND MORE',b'FOCUSED. FAST. POWERFUL.',b'CobraRecoveryPolicy',b'InfinityCobraDiagnostics',b'Switch to Infinity'):require(token in dex,'Runtime contract missing from APK: '+repr(token))
  for token in (b'ExperienceChooserUiTest',b'CobraHealthUiTest',b'CobraNavigationUiTest',b'Cobra2103159UiTest'):require(token not in dex,'Test class entered release APK')
 ...
 receipt=json.loads(Path('engine/background-resume-source.json').read_text());require(receipt['files'][SPLASH]['after']==sha(Path('kodi')/SPLASH),'Splash receipt mismatch');require(receipt['files'][ACT]['after']==patch['activity_after_sha256']==sha(Path('kodi')/ACT),'Cobra Activity receipt mismatch');require(ui.is_file(),'Experience UI ZIP missing')
 require(patch['health_center_settings_retained'] and patch['drawer_direct_infinity_handoff_removed'] and patch['power_exit_label']=='Exit','Cobra cleanup receipt contract mismatch')
 Path('audit159/apk-verification.json').write_text(json.dumps({'apk_sha256':sha(apk),'ui_zip_sha256':sha(ui),'cobra_activity_sha256':patch['activity_after_sha256'],'native_files':sum(n.startswith('lib/') for n in protected),'assets':sum(n.startswith('assets/') for n in protected),'locked_parent':2103158,'splash_bridge':1,'cobra_navigation_cleanup':1,'physical_device_verified':False},indent=2)+'\n');print('PASS: signed 2103159, locked native/assets, scoped Cobra cleanup, permanent signer and experience ZIP present')
...
def deliver():
 counts={'CobraNavigationUiTest':suite(Path('audit159/android/cobra-regression/test-results/TEST-com.projectinfinity.kodi.CobraNavigationUiTest.xml'),4),'CobraHealthUiTest':suite(Path('audit159/android/cobra-regression/test-results/TEST-com.projectinfinity.kodi.CobraHealthUiTest.xml'),7),'Cobra2103159UiTest':suite(Path('audit159/android/experience/test-results/TEST-com.projectinfinity.kodi.Cobra2103159UiTest.xml'),2),'ExperienceChooserUiTest':suite(Path('audit159/android/experience/test-results/TEST-com.projectinfinity.kodi.ExperienceChooserUiTest.xml'),6)}
 parent=list(Path('audit159/android/cobra-regression/screenshots').glob('*.png'));experience=list(Path('audit159/android/experience/screenshots').glob('*.png'));require(len(parent)>=15 and len(experience)>=6 and all(p.stat().st_size>100 for p in parent+experience),'Missing Android screenshot evidence')
 ...
 Path('signed159/ACCEPTANCE.json').write_text(json.dumps({'build':2103159,'locked_parent':2103158,'locked_commit':LOCKED,'candidate_locked':False,'apk_sha256':sha(apk),'experience_ui_version':'1.4.0','experience_ui_sha256':sha(ui),'zip_installable_by_locked_2103158_cobra_ui_installer':True,'zip_effect_on_2103158':'stored but startup chooser ignores adjunct until bridge APK is installed','native_recompiled':False,'android_test_counts':counts,'screenshots':len(parent)+len(experience),'approved_copy_present':True,'live_tv_removed_from_infinity_card':True,'hm_present':True,'cobra_drawer_direct_infinity_handoff_removed':True,'cobra_health_center_location':'Settings','cobra_power_labels':['Switch to Infinity','Exit','Cancel'],'real_device_installation_verified':False,'physical_visual_acceptance':False},indent=2)+'\n');ledger(Path('signed159'));print('PASS: APK + ZIP delivery allowed after 19 Android cases; user/device visual acceptance still pending')
