...
V=2103219;NAME='1.0.9-Cobra-Active-Section-Owner-RC1'
...
def main():
 ...
 with zipfile.ZipFile(a.apk) as z,zipfile.ZipFile(a.base) as b:
  ...
  for token in (b'cobra_active_section_owner',b'cobraSelectActiveSection',b'cobraReturnToActiveSection',
                b'cobra_smart_return_experience_display',b'cobraRestoreVodLandingReturn',
                b'LIVE TV AMBIENT BLUE',b'Infinity Health Center'):
   req(token in dex,'missing '+repr(token))
 scope=json.loads(Path('audit219/scope.json').read_text())
 for k in ('smart_return_untouched','active_section_changes_only_from_drawer','media_return_context_preserved',
           'playback_unchanged','providers_unchanged','health_center_unchanged'):req(scope.get(k) is True,'gate '+k)
 out={'build':V,'version_name':NAME,'apk_sha256':h(a.apk.read_bytes()),'smart_return_untouched':True,
      'active_section_changes_only_from_drawer':True,'native_engine_sha256':NATIVE,
      'physical_device_verified':False,'status':'TEST CANDIDATE'}
 Path('audit219/final.json').write_text(json.dumps(out,indent=2)+'\n');Path('signed219/ACCEPTANCE.json').write_text(json.dumps(out,indent=2)+'\n')
 print('PASS: 2103219 active section owner verified; Smart Return untouched')
