 ...
 * G2 Candidate 6: upper-layer adapter for the exact G2 #5 native ABI (version 2).
 * Does NOT import the separate Architecture Audited / version 3 engine.
 */
...
public final class InfinityCoreBridge {
    ...
    public static int getBridgeVersion(Main activity) {
        ...
        try {
            return activity._infinityBridgeVersion();
        } catch (LinkageError error) {
            Log.w(TAG, "Native bridge unavailable; optional bridge action skipped", error);
            return 0;
        }
    }
    ...
    public static boolean hasActiveVideoPlayer(Main activity) {
        ...
        try {
            // Deliberately preserve #5's player gate, including paused/external semantics.
            return activity._infinityHasActiveVideo();
        } catch (LinkageError error) {
            Log.w(TAG, "Video-state bridge unavailable", error);
            return false;
        }
    }
    ...
    public static void syncDisplayState(Main activity) {
        ...
        try {
            // Keep #5's existing Fold callback/native implementation unchanged.
            activity._infinitySyncDisplayState();
        } catch (LinkageError error) {
            Log.w(TAG, "Display bridge unavailable", error);
        }
    }
    ...
    public static int getSystemThemeMode(Main activity) {
        ...
        try {
            return activity._infinitySystemThemeMode();
        } catch (LinkageError error) {
            Log.w(TAG, "Theme-state bridge unavailable", error);
            return 0;
        }
    }
    ...
    public static int getNativeWindowWidth(Main activity) {
        ...
        try {
            return activity._infinityWindowWidth();
        } catch (LinkageError error) {
            Log.w(TAG, "Window-width bridge unavailable", error);
            return -1;
        }
    }
    ...
    public static int getNativeWindowHeight(Main activity) {
        if (!compatible(activity)) return -1;
        try {
            return activity._infinityWindowHeight();
        } catch (LinkageError error) {
            Log.w(TAG, "Window-height bridge unavailable", error);
            return -1;
        }
    }

    private static boolean canUsePip(Main activity) {
        // Existing callers are lifecycle callbacks on Android's main thread.
        // Do not create a new asynchronous entry path or retain an Activity reference.
    ...
    public static void updateInfinityPictureInPictureParams(Main activity) {
        try {
            ...
            // Preserve #5: explicit Home/leave entry, NOT v3 auto-entry policy.
            ...
            Log.w(TAG, "Android rejected PiP parameters; leaving normal playback alone", error);
        } catch (LinkageError error) {
    ...
    public static boolean enterInfinityPictureInPicture(Main activity) {
        try {
            if (!canUsePip(activity) || activity.isInPictureInPictureMode()
                    || !hasActiveVideoPlayer(activity)) return false;
            PictureInPictureParams params = new PictureInPictureParams.Builder()
                    .setAspectRatio(new Rational(16, 9)).build();
            return activity.enterPictureInPictureMode(params);
        } catch (IllegalArgumentException | IllegalStateException | SecurityException error) {
            Log.w(TAG, "Android rejected PiP entry; leaving normal playback alone", error);
            return false;
        } catch (LinkageError error) {
            Log.w(TAG, "PiP API unavailable", error);
            return false;
