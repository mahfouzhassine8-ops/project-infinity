...
# Keep the proven-stable 5.3.2 brain skeleton exactly as-is, then add ONLY
# Android's manifest declaration that Main supports Picture-in-Picture.
main_candidates = []
...
activity_pat = r'<activity\b[^>]*android:name="com\.projectinfinity\.kodi\.Main"[^>]*>'
...
print('5.3.3 applied: stable brain skeleton + manifest PiP capability only')
print('No PiP API calls, no Home hook, no thread, no socket, no native changes')
