...
with zipfile.ZipFile(apk) as z:
    ...
    required=(b'InfinityLiveActivity',b'InfinityCobraFeatureRuntime',b'InfinityCobraRecordingService',b'InfinityCobraReminderReceiver',b'InfinityCobraDeviceBridge',b'player_api.php?username=',b'get_live_streams',b'get_vod_streams',b'get_series_info',b'androidx/media3/exoplayer/ExoPlayer',b'RECORDINGS',b'CONTINUE WATCHING',b'cobra_custom_epg:',b'cobra-health.json',b'AUDIO / SUBS',b'CAST / ROUTE',b'PictureInPictureParams',b'infinity_player_rotation',b'.kodi/userdata/addon_data/service.infinity.refresh',b'preferredDisplayModeId',b'fold-cover',b'fold-inner',b'multiview-start',b'multiview-stop',b'player-close')
    for needle in required: assert needle in joined, needle
    for forbidden in (b'CobraTV_',b'com/cobratv',b'libmpv',b'android/media/MediaPlayer'): assert forbidden not in joined, forbidden
