...
# The preflight step owns reconstruction of the exact accepted Infinity/Cobra source
# lineage. Build stage 2 must compile that verified tree in place; it must never reset
# kodi/ to stock source or replay the version stack independently.
...
# Apply and verify the source-built GUI/render crash hardening over the already
# prepared Candidate 2 tree.
...
# Preserve source-level ownership/privacy gates immediately before the expensive
# native build so a late transform cannot reintroduce forbidden player/code paths.
...
test -s engine/overlay-inventory.json
...
with zipfile.ZipFile(apk) as z:
    ...
    assert dex, 'Candidate 2 engine contains no classes.dex payload'
    ...
    required=(
      ...
      b'InfinityCobraReminderReceiver', b'InfinityCobraDeviceBridge', b'player_api.php?username=',
      b'get_live_streams', b'get_vod_streams', b'get_series_info',
      b'androidx/media3/exoplayer/ExoPlayer', b'RECORDINGS', b'4 screens',
      b'CONTINUE WATCHING', b'cobra_custom_epg:', b'cobra-health.json', b'AUDIO / SUBS',
...
assert receipt['direct_android_gui_mutation'] is False
for key in (
  ...
  'generation_current_check_exists','commit_after_accept_api_exists','retry_api_exists'):
    assert receipt['checks'][key] is True, key

cobra=json.loads(Path('engine/cobra-full-source.json').read_text())
for key in (
  'cobra_pip','cobra_fold_reflow','cobra_multi_window',
  'cobra_rotation_shared_with_infinity','cobra_refresh_policy_shared_with_infinity'):
    assert cobra.get(key) is True, (key, cobra.get(key))
...
manifest={
  ...
  'provider_contract':['xtream','m3u'],
  'multiview_outputs':[2,3,4],
  ...
  'player':'androidx.media3.exoplayer 1.7.1',
...
Path('engine/live-app-shell-engine.json').write_text(
    json.dumps(manifest, indent=2, sort_keys=True)+'\n', encoding='utf-8')
PY

test -s engine/live-app-shell-engine.json
mkdir -p candidate
