...
# Crash diagnostics captured a repeatable startup SIGSEGV in
# CGenericTouchActionHandler::QuerySupportedGestures when touch arrives before
# the GUI service exists. Add only the null guard before Multi-View.
...
# Add the isolated Multi-View source owner after the accepted Live lineage and
# the narrow native touch startup guard.
...
grep -Fq 'if (!gui->GetWindowManager().SendMessage(msg))' kodi/xbmc/input/touch/generic/GenericTouchActionHandler.cpp
! grep -Fq 'CServiceBroker::GetGUI()->GetWindowManager().SendMessage(msg)' kodi/xbmc/input/touch/generic/GenericTouchActionHandler.cpp
...
echo 'PASS: Multi-View preflight recreated accepted Infinity stack, applied touch startup crash guard, and added isolated 2-up owner'
